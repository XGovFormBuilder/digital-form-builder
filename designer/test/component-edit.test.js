...
suite('Component edit', () => {
  ...
  test('Should display the ComponentTypeEdit component', () => {
    ...
    const wrapper = shallow(<ComponentEdit data={data} page={page} component={component} />)
  ...
  test('Should display the save and delete buttons', () => {
    ...
    expect(buttons.at(1).prop('onClick')).to.equal(wrapper.instance().onClickDelete)
  ...
  test('Should store the populated component and call callback on submit', async () => {
    ...
    const component = { type: 'TextField', name: 'myComponent' }
    const updatedComponent = { type: 'TextField', schema: { max: 24, min: 22 }, options: { required: false } }
    const wrapper = shallow(<ComponentEdit data={data} page={page} component={component} onEdit={onEdit}/>)
    ...
    expect(clonedData.updateComponent.firstCall.args[1]).to.equal(component.name)
    expect(clonedData.updateComponent.firstCall.args[2]).to.equal(updatedComponent)
