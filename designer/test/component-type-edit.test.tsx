...
suite("Component type edit", () => {
  ...
  describe("FileUploadField type gets the standard inputs but is always optional", () => {
    const cases = [
      {
        case: "with all options provided",
        options: { hideTitle: true, optionalText: false },
        hint: "a hint",
      },
      { case: "with title hidden", options: { hideTitle: true } },
      {
        case: "with title explicitly not hidden",
        options: { hideTitle: false },
      },
      { case: "with optional text hidden", options: { optionalText: false } },
      {
        case: "with optional text explicitly not hidden",
        options: { optionalText: true },
      },
    ...
    cases.forEach((testCase) => {
      test(`${testCase.case}`, () => {
        ...
        expect(
          wrapper
            .find('[data-test-id="field-options.optionalText-wrapper"]')
            .prop("hidden")
        ).to.equal(true);
  ...
  describe("Populating the component model", () => {
    componentCases.forEach((testCase) => {
      test(`${testCase.name} for ${testCase.type} component results in the appropriate model being passed to the callback function`, () => {
        ...
        const component = {
          ...testCase.componentInitialState,
          type: testCase.type,
        };
        const wrapper = mount(
          <ComponentTypeEdit
        ...
        if (field.length > 1) {
          field.first().prop(testCase.event)({
        ...
        expect(
          updateModel.firstCall.args[0],
          JSON.stringify(testCase)
        ).to.equal(testCase.expectedModel);
  ...
  const componentTypesWithValues = [
    "RadiosField",
    "CheckboxesField",
    "SelectField",
    "List",
  ...
  const inputsExcludingFileUpload = ComponentTypes.filter(
    (type) => type.subType === "field"
  ...
  inputsExcludingFileUpload.forEach((componentType) => {
    ...
    describe(`${componentType.name} type has correctly pre-populated inputs `, () => {
      const optionalCases = [
        {
          case: "with all options provided",
          options: { hideTitle: true, required: false, optionalText: false },
          hint: "a hint",
        },
        {
          case: "with optional text unspecified",
          options: { required: false },
        },
        {
          case: "with optional text hidden",
      ...
      optionalCases.forEach((testCase) => {
        test(`when specified as optional ${testCase.case}`, () => {
          ...
          const standardInputs = wrapper.find(
            '[data-test-id="standard-inputs"]'
          );
          expect(standardInputs.length).to.equal(1);
  ...
  describe("optional checkbox", () => {
    test("clicking checkbox should toggle checked", () => {
      ...
      const optionalText = wrapper.find(
