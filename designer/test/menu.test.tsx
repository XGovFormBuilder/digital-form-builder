...
suite('Page', () => {
  ...
  const flyoutGetter = (title, wrapper) => () => {
    return wrapper.find(Flyout).filterWhere(node => {
      return node.prop('title') === title
    }).first()
  }

  const getMenuButton = (title, wrapper) => {
    return wrapper.find('.menu__row').first().find('button').filterWhere(node => {
  ...
  test('sub-menu "Create new form" redirects to new form page', () => {
    const wrapper = shallow(<Menu {...props} />)
    const createNewFormMenu = wrapper.find('.menu__row').at(1).find('a').first()
