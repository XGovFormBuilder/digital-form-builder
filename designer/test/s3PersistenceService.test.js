...
suite('s3PersistenceService', () => {
  ...
  describe('listAllConfigurations', () => {
    ...
    test('should return error if one is returned', async () => {
      underTest.bucket.listObjects.returns({ promise: () => ({ error: 'some error' }) })
  ...
  describe('getConfiguration', () => {
    [
      { id: 'ab123456', expectedId: 'ab123456.json', description: 'a filename with no extension' },
      { id: 'ab123456.json', expectedId: 'ab123456.json', description: 'a filename with a json extension' },
      ...
      test(`should return configured objects when id is ${testCase.description}`, async () => {
        ...
        expect(underTest.bucket.getObject.firstCall.args[0]).to.equal({ Key: testCase.expectedId })
      })
    })

    test('should return error if one is returned', async () => {
      underTest.bucket.getObject.returns({ promise: () => ({ error: 'some error' }) })

      expect(await underTest.getConfiguration('efefsdasa')).to.equal('some error')
    })
  })
  ...
  describe('uploadConfiguration', () => {
    [
      ...
      test(`should upload configuration with a display name when id is ${testCase.description}`, async () => {
        ...
        underTest.bucket.upload.returns({ promise: () => response })
        const configuration = JSON.stringify({ someStuff: 'someValue', name: displayName })
    ...
    test('should return error if one is returned', async () => {
      underTest.bucket.upload.returns({ promise: () => ({ error: 'some error' }) })

      const configuration = JSON.stringify({ someStuff: 'someValue' })

      expect(await underTest.uploadConfiguration('my badger', configuration)).to.equal({ error: 'some error' })
    })
  })
  ...
  describe('copyConfiguration', () => {
    [
      ...
      test(`should copy configuration when id is ${testCase.description}`, async () => {
        ...
        expect(await underTest.copyConfiguration(testCase.id, newName)).to.equal(response)
    ...
    test('should return error if one is returned', async () => {
      underTest.bucket.copyObject.returns({ promise: () => ({ error: 'some error' }) })

      const configuration = JSON.stringify({ someStuff: 'someValue' })

      expect(await underTest.copyConfiguration('my badger', configuration)).to.equal({ error: 'some error' })
    })
  })
})
