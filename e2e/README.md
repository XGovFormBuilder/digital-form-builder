...
1. Ensure the runner and designer are running. It can be running locally or pointed at a URL.
   - `yarn runner dev:e2e`
   - `yarn designer dev`

The defaults are set in cypress.config.js. `DESIGNER_URL: http://localhost:3000` and `RUNNER_URL: http://localhost:3009`. You may change this via environment variables by prepending the variable name with cypress\_, e.g. `cypress_designer_url=https://..`
