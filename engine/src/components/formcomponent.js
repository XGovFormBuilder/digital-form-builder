...
export default class FormComponent extends Component {
        ...
        switch (err.type) {
          ...
          case 'date.min':
            if (limitIsToday) {
              ...
              err.message = `${err.context.label} can be no earlier than ${limit.getDate()}/${limit.getMonth() +
              1}/${limit.getFullYear()}`
            }
            break
          case 'date.max':
            if (limitIsToday) {
              ...
              err.message = `${err.context.label} can be no later than ${limit.getDate()}/${limit.getMonth() +
              1}/${limit.getFullYear()}`
            }
            break
          default:
  ...
  getViewModel (formData, errors) {
    ...
    const optionalPostfix = isOptional && options.optionalText !== false
      ? optionalText
      : ''
    this.lang = formData.lang
    const label = options.hideTitle ? '' : `${this.localisedString(
      this.title)}${optionalPostfix}`
