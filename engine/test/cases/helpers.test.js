...
suite('Helpers', () => {
  ...
  describe('redirectTo', () => {
    ...
    test('Should set params from params object', () => {
      ...
      const nextUrl = 'badgers/monkeys'
      const returned = redirectTo(request, h, nextUrl, { returnUrl: '/myreturnurl', badger: 'monkeys' })

      expect(h.redirect.callCount).to.equal(1)
      expect(h.redirect.firstCall.args[0]).to.equal(`${nextUrl}?returnUrl=%2Fmyreturnurl&badger=monkeys&f_t=myValue`)
      expect(returned).to.equal(returnValue)
    })
  ...
  describe('redirectUrl', () => {
    ...
    test('Should set params from params object', () => {
      ...
      const nextUrl = 'badgers/monkeys'
      const returned = redirectUrl(request, nextUrl, { returnUrl: '/myreturnurl', badger: 'monkeys' })

      expect(returned).to.equal(`${nextUrl}?returnUrl=%2Fmyreturnurl&badger=monkeys&f_t=myValue`)
    })
