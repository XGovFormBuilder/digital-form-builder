...
export default class ListItemReferencesToValueObjects implements Migration {
  ...
  migrateComponent(component, formDef) {
    if (
      MIGRATABLE_COMPONENT_TYPES.includes(component.type) &&
      ...
      if (list) {
        component.values = {
          type: "listRef",
          list: list.name,
          valueChildren: list.items
            .filter((item) => item.conditional?.components)
            .map((item) => ({
              value: item.value,
              children: item.conditional.components.map((it) =>
                this.migrateComponent(it, formDef)
              ),
            })),
        };
        ...
        this.logger.error(
          `Unable to migrate component with list name ${listName} as the corresponding list does not exist`
        );
      }
    }
    return component;
