...
export class DatePartsField extends FormComponent {
  ...
  constructor(def: InputFieldsComponentsDef, model: FormModel) {
    ...
    const isRequired = "required" in options && options.required === false ? false : true;
  ...
  getViewModel(formData: FormData, errors: FormSubmissionErrors) {
    ...
    const isRequired = "required" in this.options && this.options.required === false ? false : true;
    ...
    if (missingParts.length === 3) {
      text = `${this.title} must be a valid date.`;
